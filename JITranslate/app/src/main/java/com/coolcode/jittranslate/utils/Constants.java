package com.coolcode.jittranslate.utils;

public class Constants {
    public static final String testFb2File = "Diane_Setterfield-The_Thirteenth_Tale.fb2";
    public static final String testBookName = "Don Kihot";
    public static final String testBookAuthor = "Migel Servantes";
    public static final String clientsBooksCoversDir = "client_books_covers";
    public static final String clientsBooksDir = "client_books";
    public static final String assetsBooksCoversDir = "covers";
    public static final String assetsBooksDir = "books";
    public static int screenHeight;
    public static int screenWidth;
    public static int currentTextSize;
}
